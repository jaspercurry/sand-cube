"""Build and validate the 140-degree rollback horn variant."""
...
def main() -> None:
    ...
    checks: list[dict[str, Any]] = [
        ...
        {
            "name": "de250_throat_not_constricted",
            "passed": PARAMS.throat_d >= PARAMS.official_de250_exit_d,
            "actual": PARAMS.throat_d,
            "minimum": PARAMS.official_de250_exit_d,
        },
    ]
    _check(
        checks,
    ...
    _check(
        checks,
        "acoustic_mouth_inner_d",
        float(profile["mouth_inner_d_mm"]),
        2.0 * PARAMS.mouth_inner_r,
        0.001,
    )
    _check(
        checks,
    ...
    _check(
        checks,
        "cad_spline_throat_tangent",
        float(profile["cad_spline_throat_tangent_deg"]),
        PARAMS.throat_angle_deg,
        0.001,
    )
    _check(
        checks,
    ...
    step_path = out / "le_cleach_horn_190_rollback_140.step"
    ...
    diagnostics = {
        "design": "le_cleach_horn_190_rollback_140",
    ...
    if not passed:
        raise SystemExit("140-degree rollback geometry contract failed")
